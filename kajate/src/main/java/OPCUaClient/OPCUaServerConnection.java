...
public class OPCUaServerConnection {
    ...
    {
        {
            adminTags.put("ProdProcessedCount", "::Program:Cube.Admin.ProdProcessedCount"); // Total amount of produced
                                                                                            // products
            adminTags.put("ProdDefectiveCount", "::Program:Cube.Admin.ProdDefectiveCount"); // Total amount of defective
                                                                                            // products
            adminTags.put("StopReasonId", "::Program:Cube.Admin.StopReason.Id"); // [10]Empty Inventory;[11]Maintenance
                                                                                 // Needed;[12]Manual Stop;[13]Motor
                                                                                 // Power Loss;[14]Manual Abort
            adminTags.put("StopReasonValue", "::Program:Cube.Admin.StopReason.Value"); // not sure.. could be the
                                                                                       // descriptions that fit the
                                                                                       // above id's
    ...
    {
        {
            ...
            commandTags.put("Type", "::Program:Cube.Command.Parameter[1].Value"); // float | Type of next batch |
                                                                                  // [0]pilsner;[1]wheat;[2]IPA;[3]Stout;[4]Ale;[5]Alcohol-free
            commandTags.put("Amount", "::Program:Cube.Command.Parameter[2].Value"); // float | Amount of products in the
                                                                                    // next batch
            commandTags.put("CntrlCmd", "::Program:Cube.Command.CntrlCmd"); // int |
                                                                            // [1]reset;[2]start;[3]stop;[4]abort;[5]clear
            commandTags.put("CmdChangeRequest", "::Program:Cube.Command.CmdChangeRequest"); // bool | [true]starts
                                                                                            // system with given
                                                                                            // inputs;[false]idle
    ...
    private void subscribeToEndpoint(String identifier) {
        try {
            ...
            MonitoringParameters parameters = new MonitoringParameters(clientHandle, 1000.0, null, Unsigned.uint(10),
                    true);
            // Creation request
            ...
            BiConsumer<UaMonitoredItem, Integer> onItemCreated = (item, id) -> item
                    .setValueConsumer(OPCUaServerConnection::onSubscriptionValue);
            // Subscribe: create a subscription @ 1000ms
            UaSubscription subscription = getInstance().connectToOPCUAServer().getSubscriptionManager()
                    .createSubscription(1000.0).get();
            List<UaMonitoredItem> items = subscription
                    .createMonitoredItems(TimestampsToReturn.Both, Arrays.asList(request), onItemCreated).get();
            ...
            double waitTime = ((readEndPoint(statusTags.get("Products"))/Speed.get("tfMachineSpeed").getAsFloat())*60000)+3000;
    ...
    private static void onSubscriptionValue(UaMonitoredItem item, DataValue value) {
        ...
        getInstance().dataset.put("OEE", getInstance().OOECalculation()); 
        getInstance().dataset.put("erCalc",getInstance().ErrorCacalculation());
        ...
        getInstance().logs.get("Temperature")
                .add(getInstance().readEndPoint(getInstance().statusTags.get("Temperature")));
        getInstance().logs.get("Vibration").add(getInstance().readEndPoint(getInstance().statusTags.get("Vibration")));
        getInstance().logs.get("ProdDefectiveCount")
                .add(getInstance().readEndPoint(getInstance().adminTags.get("ProdDefectiveCount")));
        ...
        System.out.println("OOE" + getInstance().dataset.get("OEE"));
    ...
    // idealCycleTime = machinespeed products per min
    // total count , rejectcount default 0,
    // getInstance().readEndPoint(adminTags.get("ProdDefectiveCount")
    private double OOECalculation() {
        ...
        double OOE = ((goodCount * idealCycleTime) / plannedProductionTime);
        return OOE;
    }
    
         // The error would be the difference between the input amount and the acceptable amount..
    private double ErrorCacalculation() {
         float goodCount = getInstance().readEndPoint(statusTags.get("Products"))
         - (float) (Math.random() * (10 - 1) + 1);
